...
| **知识库健康（Knowledge Health）**                             | 对知识库结构完整性 / 索引漂移 / 积压 / 分布的量化诊断                                                                                                                                                 | 诊断层     |
| **索引漂移（Index Drift）**                                    | `knowledge-index.md` 与实际 `knowledge/` 条目不一致（双向孤儿）                                                                                                                                       | 诊断层     |
| **候选线索（Candidate Clue）**                                 | Git 历史提取的产物（pattern + 建议 type + 支撑样本），非成型条目                                                                                                                                      | 中间产物   |
| **复审调度（Review Scheduling）**                              | 待复审名单按 `updated` 升序优先（先看最久未复审的），非自动降档                                                                                                                                       | 诊断层     |
| **归一化全等重复检测（Normalized Exact-Duplicate Detection）** | 归一化（小写+去空白+去标点）后**整段全等**判定重复条目（**非语义近重复**），跨 `knowledge/` 与 `.pending/`，作用域内分扫，由 `doctor`/`boot` 表面化、`--fix` 逐条人工三选合并（ADR 0005），非实时拦截 | 诊断层     |
...
> **`convention` vs `architecture` 划分判据**：这条知识 _AI 光看代码能否自己猜出来？_
...
> **Git 历史提取已落地为确定性启发式（ADR 0004）**：`evokit learn --git-history` 生成候选线索到 `.pending/`（`source=git-history`），确认仍走无参 `evokit learn`。启发式遵循 `docs/research/git-history-heuristics.md`（结构匹配率门控 + 排除清单），"漏提优于误提"。
...
> **作用域判定判据**：问自己——"这条知识 _换到另一个项目还成立吗？_"
...
> 助手间同条目冲突合并落地为**事后健康诊断**（非实时协商）——见 ADR 0005：对话提取/确认是 AI 直接写挪文件、无代码 chokepoint 可注入写入时去重，且"实时"需 daemon/IPC 违反"本地优先"。v1.2 显式取舍不修改 AI-direct skill 协议，故由 `doctor`/`boot` 表面化归一化全等重复条目（**非语义近重复**，语义近似推 v1.3）、`--fix` 逐条人工三选合并（绝不自动择主），与索引漂移检测同构。
...
> **确认闸门去重**：**显式声明**时，若同 `type` 已存在归一化内容（去空白/大小写/标点）相近的 active 条目，提示"已有相似条目 X"，用户三选（覆盖已有 / 仍新建 / 放弃）。只做归一化匹配，不引入 embedding 语义去重。确认（对话提取）路径靠既有跨区精确 id 去重（`uniqueDraftId`），不追加模糊提示。
...
> **算法落地 = 知识库健康诊断 + 复审调度（ADR 0003）**。降档仍由人工/AI 二分判断驱动，系统新增一个**诊断层**主动表面化该被复审的条目：
>
> - **索引漂移检测** — `knowledge/` 与索引双向孤儿，`evokit doctor` / `evokit boot` 报告，`--fix` 重建索引。
> - **健康指标** — frontmatter 合法性、`.pending/` 积压、STALE/RETIRED 积压、条目分布（by scope/type/confidence）。
> - **复审调度** — `/evokit-review` 待复审名单 confidence 升序（RETIRED 在前）+ 同档内按 `updated` 升序（先看最久未复审的），是优先级提示而非自动降档。
> - **明确不做** — 时间自动衰减、代码扫描/AST 失效判定、embedding 语义过期检测（见 ADR 0003 取舍）。
