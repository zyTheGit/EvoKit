...
这会将 EvoKit 模板安装到项目目录，配置自定义工具（`evokit-boot.ts`、`evokit-session.ts` 等）替代生命周期钩子。
...
这会将 EvoKit 模板安装到 `~/.pi/agent/`，配置 TypeScript 扩展（`evokit-lifecycle.ts` 等）处理生命周期事件。Pi CLI 的引导验证和会话记录是自动的。
