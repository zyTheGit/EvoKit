...
EvoKit 通过**项目上下文引擎**让规范相关的项目/个人知识持续积累：

### 知识生命周期

```
对话中识别知识 → 静默写入 <project>/.evokit/.pending/
    ↓（确认时人工裁定 scope）
确认背书 → 移入 knowledge/（个人入 ~/.evokit/knowledge/、项目留 <project>/.evokit/）
    ↓
索引更新（knowledge-index.md）→ 后续会话 AI 一次查询覆盖所有知识
```
...
| 命令              | 作用                     | 频率                       |
| ----------------- | ------------------------ | -------------------------- |
| `/evokit-boot`    | 知识库完整性检查（只读） | 每次会话启动（自动）      |
| `/evokit-learn`   | 确认背书 / 显式声明      | 有 `.pending/` 待确认时   |
| `/evokit-review`  | 复审过期知识（conf ≤ 0.5） | 定期                   |

### 知识如何沉淀

1. 你纠正 AI：**"用 const，不要用 var"**（确定性新事实）
2. AI 静默写入 `.pending/`（不猜 scope）
3. 你运行 `evokit learn` 确认 → 入库并更新索引
4. 个人知识入 `~/.evokit/knowledge/`，后续所有助手（4 助手共享）都能读到
...
- **识别到专属知识就写入 `.pending/`**——但不要猜 scope，留到确认时裁定
- **定期运行 `/evokit-review`**——审查过期的架构/约定知识
- **把规范纳入 code review**——规范也是代码，需要维护

### ❌ 不应该做的

- 不要在 CLAUDE.md 中堆砌细节——那是 rules/ 或知识库的工作
- 不要写一次性命令 / 通用常识 / 未确认的猜测到 `.pending/`（防误提取）
- 不要把个人偏好写成全局规则——用 scope 限定作用域
- 不要忽略 `/evokit-boot` 的失败项——它们是知识库健康的信号

### 团队协作建议

1. 将 `<project>/.evokit/`、`.claude/rules/` 和 `CLAUDE.md` 纳入 git 跟踪
2. 规范变更走 PR 审查（和代码一样）
3. 定期在团队会议中审查项目知识——清理不再适用的条目
